...
import {
  ...
  Pencil, Check, Search, Plus, Eye, Tag, Music, LogOut,
...
// Shared input style — explicitly white bg + dark text to defeat iOS Safari dark-mode overrides
const inputStyle = {
  width: "100%",
  padding: "10px 14px",
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, zIndex: 100, background: "#fff", border: "1px solid #E8E4DE", borderRadius: 12, boxShadow: "0 8px 32px rgba(28,26,23,0.12)", minWidth: 200, overflow: "hidden" }}>
          ...
          <div style={{ maxHeight: 220, overflowY: "auto" }}>
            {filtered.length === 0
              ? <p style={{ padding: "12px 14px", fontSize: 12, color: "#A09890", margin: 0, fontFamily: "'Satoshi', sans-serif" }}>No tags found</p>
              : filtered.map(tag => {
                  const active = selectedTags.includes(tag.name);
                  return (
                    <button key={tag.id} onClick={() => onToggle(tag.name)}
                      style={{ width: "100%", textAlign: "left", padding: "9px 14px", border: "none", background: active ? "#F5EDD8" : "transparent", color: active ? "#B8922A" : "#1C1A17", cursor: "pointer", fontFamily: "'Satoshi', sans-serif", fontSize: 13, fontWeight: active ? 600 : 400, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      {tag.name}
          ...
          {selectedTags.length > 0 && (
            <div style={{ padding: "8px 12px", borderTop: "1px solid #E8E4DE" }}>
              <button onClick={() => { onClear(); setOpen(false); }}
                style={{ width: "100%", padding: "7px 0", background: "#F7F5F2", border: "1px solid #E8E4DE", borderRadius: 8, fontSize: 12, color: "#A09890", cursor: "pointer", fontFamily: "'Satoshi', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
                <X size={10} /> Clear all
              </button>
            </div>
          )}
        ...
        {step === "recovery" ? (
          <div style={{ padding: 32, display: "flex", flexDirection: "column", gap: 20, textAlign: "center" }}>
            <div>
              ...
              <h2 style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 20, fontWeight: 700, color: "#1C1A17", margin: "0 0 8px" }}>Admin Created!</h2>
          ...
          <>
            <div style={{ padding: "22px 24px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 19, fontWeight: 700, color: "#1C1A17" }}>Add Admin</span>
              </div>
            ...
            <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ padding: "10px 14px", background: "#F5EDD8", borderRadius: 10, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 11, color: "#B8922A", fontWeight: 500 }}>This account will have full admin access</span>
              </div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(28,26,23,0.4)", backdropFilter: "blur(6px)" }}>
      <div style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 440, margin: "0 16px", boxShadow: "0 32px 80px rgba(28,26,23,0.18)", overflow: "hidden" }}>
        <div style={{ padding: "22px 24px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 19, fontWeight: 700, color: "#1C1A17" }}>{tabLabel}</span>
          <button onClick={onClose} style={{ width: 34, height: 34, borderRadius: 10, border: "1px solid #E8E4DE", background: "#F7F5F2", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#6B6560" }}>
            <X size={15} />
          </button>
        </div>
        <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ position: "relative", background: "#FAFAF9", border: "2px dashed #D4CFC8", borderRadius: 14, padding: "28px 20px", textAlign: "center", cursor: "pointer" }}>
            ...
            {files.length > 0
              ? <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 13, color: "#B8922A", fontWeight: 500, margin: 0 }}>{files.length} file{files.length !== 1 ? "s" : ""} selected</p>
              : <>
                  <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 13, color: "#1C1A17", fontWeight: 500, margin: "0 0 3px" }}>Click to browse</p>
                  <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 12, color: "#A09890", margin: 0 }}>
            ...
            <div style={{ border: "1px solid #E8E4DE", borderRadius: 12, overflow: "hidden", maxHeight: 140, overflowY: "auto" }}>
              {files.map((f, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderBottom: i < files.length - 1 ? "1px solid #E8E4DE" : "none" }}>
                  <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 11, color: "#1C1A17", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.name}</span>
                  <span style={{ fontFamily: "monospace", fontSize: 10, color: "#A09890", flexShrink: 0 }}>{(f.size / (1024 * 1024)).toFixed(1)} MB</span>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(28,26,23,0.4)", backdropFilter: "blur(6px)" }}>
      <div style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 420, margin: "0 16px", boxShadow: "0 32px 80px rgba(28,26,23,0.18)", overflow: "hidden" }}>
        <div style={{ padding: "22px 24px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 19, fontWeight: 700, color: "#1C1A17" }}>Edit Book</span>
          <button onClick={onClose} style={{ width: 34, height: 34, borderRadius: 10, border: "1px solid #E8E4DE", background: "#F7F5F2", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#6B6560" }}>
            <X size={15} />
          </button>
        </div>
        <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Book title..." style={inputStyle} />
          </div>
          <div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={onClose} style={{ flex: 1, padding: "12px 0", background: "#F7F5F2", color: "#6B6560", border: "1px solid #E8E4DE", borderRadius: 10, fontFamily: "'Satoshi', sans-serif", fontSize: 13, cursor: "pointer" }}>
              Cancel
            </button>
          </div>
        </div>
      ...
      <div style={{ position: "relative", aspectRatio: "2/3", background: "#F7F5F2", overflow: "hidden" }}>
        {book.cover_url
          ...
          : <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: 16 }}>
              ...
              <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 11, color: "#A09890", textAlign: "center", lineHeight: 1.4 }}>{book.title}</span>
            </div>
        }
        <div style={{ position: "absolute", top: 8, left: 8 }}>
          <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 10, fontWeight: 600, padding: "3px 8px", borderRadius: 20, background: book.extension === "epub" ? "#F0FAF4" : "#FEF2F0", color: book.extension === "epub" ? "#2D7A4F" : "#D94F3D", textTransform: "uppercase", letterSpacing: "0.03em" }}>
            {book.extension}
          </span>
        </div>
        ...
        {book.tags?.length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
            {book.tags.slice(0, 3).map(tag => (
              <span key={tag.id || tag.name} style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 10, padding: "2px 8px", borderRadius: 20, background: "#F5EDD8", color: "#B8922A", fontWeight: 600 }}>
                {tag.name}
              </span>
            ))}
            {book.tags.length > 3 && <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 10, color: "#A09890" }}>+{book.tags.length - 3}</span>}
          </div>
        )}
      </div>
    </div>
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.92)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 960, display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 15, fontWeight: 500, color: "#F0EAD6", margin: 0 }}>{video.title}</p>
        ...
        {video.description && <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 13, color: "#A09890", margin: 0 }}>{video.description}</p>}
        {video.tags?.length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {video.tags.map(tag => (
              <span key={tag.id || tag.name} style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 11, padding: "3px 10px", borderRadius: 20, background: "rgba(255,255,255,0.1)", color: "#A09890" }}>
                {tag.name}
              </span>
            ))}
          </div>
...
const VideoCard = ({ video, onDelete, onUpdate, isAdmin, onPlay }) => {
  ...
  const editInputStyle = { ...inputStyle, padding: "9px 12px", borderRadius: 9 };
    ...
    <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden", border: "1px solid #E8E4DE", boxShadow: hovered ? "0 8px 28px rgba(28,26,23,0.10)" : "0 1px 4px rgba(28,26,23,0.05)", transition: "all 0.2s ease" }}>
      ...
      {editing ? (
        <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 8 }}>
          <input autoFocus value={editTitle} onChange={e => setEditTitle(e.target.value)} placeholder="Title..." style={{ ...editInputStyle, border: "1.5px solid #B8922A" }} />
          <input value={editDesc} onChange={e => setEditDesc(e.target.value)} placeholder="Description (optional)..." style={{ ...editInputStyle, fontSize: 12, color: "#6B6560", WebkitTextFillColor: "#6B6560" }} />
          <input value={editTags} onChange={e => setEditTags(e.target.value)} placeholder="Tags (comma separated)..." style={{ ...editInputStyle, fontSize: 12, color: "#6B6560", WebkitTextFillColor: "#6B6560" }} />
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={saveEdit} style={{ flex: 1, padding: "9px 0", background: "#B8922A", color: "#fff", border: "none", borderRadius: 9, fontFamily: "'Satoshi', sans-serif", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              <Check size={12} /> Save
            </button>
            <button onClick={() => setEditing(false)} style={{ flex: 1, padding: "9px 0", background: "#F7F5F2", color: "#6B6560", border: "1px solid #E8E4DE", borderRadius: 9, fontFamily: "'Satoshi', sans-serif", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              <X size={12} /> Cancel
            </button>
          </div>
        </div>
        ...
        <div style={{ padding: "10px 14px 12px" }}>
          <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 13, fontWeight: 500, color: "#1C1A17", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", margin: "0 0 4px" }}>{video.title}</p>
          {video.tags?.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
              ...
              {video.tags.length > 3 && <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 10, color: "#A09890" }}>+{video.tags.length - 3}</span>}
            </div>
          )}
        </div>
      )}
      ...
      {editing ? (
        ...
        <div style={{ padding: "12px 14px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, borderTop: "1px solid #E8E4DE" }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 13, fontWeight: 500, color: "#1C1A17", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", margin: "0 0 4px" }}>{audio.title}</p>
            {audio.tags?.length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                ...
                {audio.tags.length > 3 && <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 10, color: "#A09890" }}>+{audio.tags.length - 3}</span>}
              </div>
            )}
          </div>
          {isAdmin && (
...
const EmptyState = ({ tab, onUpload, isAdmin }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 16, textAlign: "center" }}>
      ...
      <div>
        <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 20, fontWeight: 700, color: "#1C1A17", margin: "0 0 6px" }}>No {tab} yet</p>
        <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 13, color: "#A09890", margin: 0 }}>
          {isAdmin ? `Upload some ${tab} to get started` : "Nothing here yet — check back later"}
        </p>
      </div>
      {isAdmin && (
        <button onClick={onUpload} style={{ padding: "11px 22px", background: "#B8922A", color: "#fff", border: "none", borderRadius: 10, fontFamily: "'Satoshi', sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
...
const Library = () => {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#fff", fontFamily: "'Satoshi', sans-serif", overflow: "hidden" }}>
      ...
      <aside style={{ width: isMobile ? 58 : 220, flexShrink: 0, background: "#fff", borderRight: "1px solid #E8E4DE", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: isMobile ? "14px 0" : "24px 20px 20px", borderBottom: "1px solid #E8E4DE", display: "flex", alignItems: "center", justifyContent: isMobile ? "center" : "flex-start" }}>
          {isMobile
            ? <BookOpen size={20} color="#B8922A" />
            : <div>
                <h1 style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 21, fontWeight: 700, color: "#1C1A17", margin: 0, letterSpacing: "-0.01em" }}>Jirani</h1>
                <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 10, color: "#A09890", margin: "4px 0 0", textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 600 }}>Offline Library</p>
              </div>
          }
        </div>
        ...
        <div style={{ marginTop: "auto", padding: isMobile ? "10px 6px" : "12px 10px", borderTop: "1px solid #E8E4DE", display: "flex", flexDirection: "column", gap: 6 }}>
          {isAdmin && (
            <button onClick={() => setShowAddAdmin(true)}
              ...
              style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: isMobile ? "center" : "flex-start", gap: 8, padding: isMobile ? "8px 0" : "8px 12px", borderRadius: 10, border: "1px dashed #D4A93A", background: "transparent", color: "#B8922A", cursor: "pointer", fontFamily: "'Satoshi', sans-serif", fontSize: 12, fontWeight: 600 }}>
          ...
          <div style={{ padding: isMobile ? "8px 0" : "8px 12px", borderRadius: 10, background: "#FAFAF9", display: "flex", alignItems: "center", justifyContent: isMobile ? "center" : "space-between", gap: 8 }}>
            {!isMobile && (
              <div style={{ minWidth: 0 }}>
                <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 12, fontWeight: 600, color: "#1C1A17", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {isAdmin ? auth?.username : "Student"}
                </p>
                <p style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 10, color: "#A09890", margin: 0, textTransform: "uppercase", fontWeight: 600 }}>
                  {isAdmin ? "Admin" : "Guest"}
                </p>
              </div>
            )}
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, background: "#FAFAF9" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: isMobile ? "12px 12px" : "16px 28px", background: "#fff", borderBottom: "1px solid #E8E4DE" }}>
          <h2 style={{ fontFamily: "'Satoshi', sans-serif", fontSize: isMobile ? 16 : 22, fontWeight: 700, color: "#1C1A17", margin: 0, letterSpacing: "-0.01em", flexShrink: 0 }}>
            {activeTab === "books" ? "Books" : activeTab === "audio" ? "Audio" : "Videos"}
          </h2>
          <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: 11, padding: "2px 9px", borderRadius: 20, background: "#F5EDD8", color: "#B8922A", flexShrink: 0, fontWeight: 600 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {isAdmin && (
              <button onClick={() => setShowUpload(true)}
                ...
                style={{ display: "flex", alignItems: "center", gap: isMobile ? 0 : 7, padding: isMobile ? "8px 10px" : "9px 16px", background: "#fff", border: "1.5px solid #B8922A", borderRadius: 10, fontFamily: "'Satoshi', sans-serif", fontSize: 13, fontWeight: 600, color: "#B8922A", cursor: "pointer", transition: "background 0.15s", whiteSpace: "nowrap", flexShrink: 0 }}>
                <Upload size={13} />
